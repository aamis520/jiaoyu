    ...
    export default {
        ...
        methods: {
            ...
            getSchool(area_id, head_school_id, index) {
                ...
                let obj ={
                    areaId : area_id,
                    headSchoolId :head_school_id
                }
//                getSchType(area_id,head_school_id);
                this.axios.post('/api/api-interconnection/v1/school/findSchoolList',obj).then((res) => {
                    let haveOrNo = false;
                    this.concatAr =[];
                    ...
                    if(haveOrNo != true){
                        for(let i =0;i<res.messages.data.length;i++){
                            res.messages.data[i].bool = false;
                        }
                        this.conArrAll = this.conArrAll.concat(res.messages.data);
            ...
            fn() {
                return new Promise((success, err) => {
                    ...
                    this.axios.post('/api/api-interconnection/v1/school/findAreaList',obj)
                        .then(res => {
                            success(res)
                        })
                        .catch(error => {
            ...
            courList(item,e,index){
                ...
                if(item.bool == false){
                    item.bool = true;
                    this.arrEnd.push({school_id:item.school_id,school_name:item.school_name})
        ...
        mounted() {
            ...
            this.fn().then(function (res) {
                _this.area = res.messages.data;
                let obj = {
                    areaId : res.messages.data[0].area_id,
                    headSchoolId:res.messages.data[0].head_school_id
                }
                _this.axios.post('/api/api-interconnection/v1/school/findSchoolList',obj).then((res) => {
                    if(_this.concatAr.length == 0){
                        for(let i =0;i<res.messages.data.length;i++){
                            res.messages.data[i].bool = false;
                        }
                        _this.conArrAll = _this.conArrAll.concat(res.messages.data);
...
//                    _this.campus = res.messages.data;
                }).catch(function (error) {
                    console.log(error)
                })
            })
