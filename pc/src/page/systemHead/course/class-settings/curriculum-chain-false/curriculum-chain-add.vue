	...
	export default {
		...
		methods: {
			listClick(index, ele) { //选择类型
				...
				this.$store.dispatch('getClassSetIdTypeListFalseser', {
					id: ele.id,
					name: ele.name
				})
			...
			Ajax(a) {
				...
				if(this.falselist.chain_course_list.length == 0) {//判断当前是否为第一次添加非课程链
					console.log("nihao")
					this.$store.dispatch('addClassSetNewListfalseser', this.obj)
						.then(() => {
							if(a == 2) {//判断是继续追加还是结束本链条
								this.$store.dispatch('getClassSetFalseListser')
							} else {
								this.$store.dispatch('getClassSetIdTypeListFalseser', {//继续添加后刷新页面
									name: this.checkName,
									id: this.classId
								})
							}
						})
				} else {//如果是第二次添加，则调用追加的接口
					newobj.class_course_chain_id = this.falselist.chain_course_list.id
					newobj.chain_or_no = "noChain"
					this.$store.dispatch('addClassSetNewListser', newobj)
					.then((res)=>{
						if(a == 2) {//判断是继续追加还是结束本链条
								this.$store.dispatch('getClassSetFalseListser')
							} else {
