...
<script>
	...
	import addClass from './on-n-settings-add'
	export default {
		data() {
			return {
				show: false, 
		...
		methods: {
			...
			imgClick(index){
				this.$store.commit('GET_ONNSETTINGSCLASSSHOW',index) 
			},
			removeClick(index1,index2,ele){
				this.$store.commit('REMOVE_ONNLIST',{
					index1,
					index2,
					id: ele.id
				})  
		...
		mounted() {
			this.$store.dispatch('getVipNClassList')
		}
	}
</script>
