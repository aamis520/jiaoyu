	...
	<div class="add_classroom" :style="{ width: windowWidth + 'px' }">
		<section>
			<div class="add_classroom_first">
				创建教室
				<span @click="closeRoomClick()"><img src="../../../../images/system-campus/close.jpg" alt="" /></span>
			</div> 
						<!--必需排座  -->
						<ul id="table_list_te" v-if="false">
							<li v-for="(ele,index) in formData" :key="index">
								...
								<span class="span_input_small span_last_img">
                                    <img class="last_img" src="../../../../images/system-campus/sds.png" @click="remList(index)"></img>
                                    <img class="last_img" src="../../../../images/system-campus/add5.png" @click="addList(ele.isImgShow , index )" v-if="ele.isImgShow"></img>
                                </span>
							</li>
						...
						<ul id="table_list_teno" v-if="isno">
							<li v-for="(ele,index) in formData2" :key="index">
								...
								<span>
                                    <img class="last_img" src="../../../../images/system-campus/sds.png" @click="remList2(index)" v-if="index != 0"></img>
                                    <img class="last_img" src="../../../../images/system-campus/add5.png" @click="addList2(ele.isImgShow , index )" v-if="ele.isImgShow && !isxiugai"></img>
                                </span>
							</li>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// },
			submitOk2() {
				let pageInitUrl = cont.campus + "v1/campus/classroom/insertOrUpdateClassRoomInfo"
				/**
				 * 判断是否为添加
				 */
				if(this.newdata.isxiugai) {
					...
					this.formData2.map((ele, index) => {
						...
						newobj.type = "class"
						newobj.school_id = this.schoolId
				...
				var newlist = {
					classRoomListStr : pageInitparmes
				}
				/**
				 * 先简单判断不能为空才请求
				 */ 
				if(this.formData2[0].room_name != "" && this.formData2[0].valid_num != "" && this.formData2[0].valid_num > 0) {
					var that = this
					this.axios.post(pageInitUrl, newlist)
						.then(res => { 
								/**
								 * 判断班级是否重名
								 * */
								if(res.errorcode == 1004004) {
