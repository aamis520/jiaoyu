	...
	<div class="costGradesBg">
		<div class="container">
			...
			<div class="content">
				...
				<ul class="check_box">
					<li>
						<span class="col">适用类型</span>
						<span class="item2" v-for="(ele,index) in applyTypeDate" @click="toggleShow(applyTypeDate,index)" :key="index">{{ele.name}}<i v-if="ele.check" class="ico-init ico-duigou2"></i></span>
					</li>
					<li>
	...
	export default {
		data() {
			return {
				...
				applyTypeDate:[//适用班型数据
					{id:0,name:'一对二',check:false},
					{id:0,name:'一对三',check:false},
					{id:0,name:'一对四',check:false},
					{id:0,name:'一对五',check:false},
					{id:0,name:'6-10人班',check:false}
				],
		...
		methods:{
			...
			getOtherData(){
                ...
                this.applyTypeDate.map((ele,index)=>{
                	if (ele.check) {
                		this.costGradesDate.applyType.push(ele)
                	}
                })
			},
			sendDate(str){
