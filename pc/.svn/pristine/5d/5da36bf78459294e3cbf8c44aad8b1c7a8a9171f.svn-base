	...
	<div>
		...
		<div class="curriculum-falselistwai">
			<div class="curriculum-falselist">
				...
				<ul>
					<li v-for="(ele,index) in truelist.list" :key="index">
						<h2>{{ele.name}}（{{ele.code}}）</h2>
						<section>
							<span v-for="(ele2,index2) in ele.chain_course_list" :key="index2" :class="{length_class: index2 != ele.chain_course_list.length-1 , is_span : ele.is_delete == false}" @click="bianji(ele2,index2,index)">{{ele2.course_name}} · {{ele2.course_num}}期 · {{ele2.class_type_list.length}}个班型</span>
						</section>
					</li>
				</ul>
			</div>
	...
	export default {
		...
		computed: {
			...mapState({
				...
				truelist: state => state.classSettings.truelist
			})
		},
		methods: {
			...
			bianji(ele, index2, index) {
				if(ele.is_delete) {
					...
					this.classId = this.truelist.list[index].id  
					this.sort_num = ele.sort_num
					if(index2 == this.truelist.list[index].chain_course_list.length - 1) {
						this.btnshow = true
					} else {
						this.btnshow = false
					...
					.then(() => {
						...
						setTimeout(() => {
							this.$store.dispatch('getClassFalseListser', {
								name: this.truelist.name,
								id: this.truelist.list[0].subject.id,
								chain: "chain"
							})
						}, 500)
					})
