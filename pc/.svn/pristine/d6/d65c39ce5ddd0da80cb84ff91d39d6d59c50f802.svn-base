	...
	<div id="incomeForm">
		<div class="formTitle">
			...
			<div class="titleRight">
				...
				<span class="tRleft" @click="getCST(-1)"><img src='../../../images/finance/arrowLeft.png'/></span>
				<span class="tRright" @click="getCST(1)"><img src="../../../images/finance/arrowRight.png"/></span>
				<span class="tRset" style="display: none;"><img src="../../../images/finance/set.png"/></span>
				<span class="tRtex" style="display: none;"><img src="../../../images/finance/illustrate.png"/></span>
				<span class="tRserch" style="display: none;"><img src="../../../images/finance/search.png"/></span>
			</div>
		...
		<div class="shouruReport">
			<table class="tableClass" border="0" cellspacing="0" cellpadding="0">
				<tr v-for="(val, key, index) in dayForm" :class="{'bgGray':index%2 != 0}">
					<td class="tdColorBlack">{{key}}<img src="../../../images/finance/incomeReport.png" v-if="headSchoolBoolean" @click.stop.prevent="showDetail(key)" /></td>
...
//				let url = cont.finance + 'v1/financestat/financialincome/selectIncomeStatements?accessToken=aaaa&headSchoolId=' + this.headSchoolId + '&queryMonth=' + newCST
				...
				let url = cont.finance + 'v1/financestat/financialincome/selectIncomeStatements'
				this.axios.post(url,obj).then((res) => {
					if(!res.messages.data) {
						...
						let newDay = {}
						let newDayTotal = {}
						for(let i in res.messages.data) {
							if(i !== 'total') {
								newDay[i] = res.messages.data[i]
							} else {
								newDayTotal.total = res.messages.data[i]
							}
						}
						this.dayForm = newDay
			...
			showDetail(time) {
				...
				let obj={
					headSchoolId:this.headSchoolId,
					queryDay:this.timeFormat
				}
				let url = cont.finance + 'v1/financestat/financialincome/selectSchoolIncomeStatements'
				this.axios.post(url,obj).then((res) => {
					if(!res.messages.data) {
						this.FormLayer = {}
						this.FormLayerTotal = {}
