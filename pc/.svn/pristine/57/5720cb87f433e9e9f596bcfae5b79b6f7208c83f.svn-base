...
export default {
	...
	methods: {
		 ...
		 */
		pageInit(valid_num) {
			let pageInitUrl = cont.campus+"v1/campus/classroom/selectClassRoomBySchoolId?head_id=" + this.$ls.get("USER_INFO").headSchoolId
			let pageInitparmes = { 
				pageSize: 10,
				pageNo: valid_num, 
			...
			this.axios.post(pageInitUrl, pageInitparmes)
				.then(res => {
					...
					this.list1.total = res.messages.data.totalCount 
					this.list1.data = res.messages.data.datas 
				},
				res => {
					alert("网络繁忙 ！请刷新重试 ！")
				})
