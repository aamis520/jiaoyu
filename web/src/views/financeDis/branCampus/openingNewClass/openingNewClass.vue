	...
	export default {
		...
		methods: {
	        ...
	        keLianAjax(id){
	        	...
	        	let url = cont.campus+"v1/campus/coursechain/slectClassCourseChainInfo?accessToken=aaaa"
	            ...
	            .then(res => {     
						 this.listData.dataArray = res.data.messages.data  
	                },
	                res => {
	                    alert("网络繁忙 ！请刷新重试 ！")
	                })
	         ...
	         * */ 
	        addInit(id , listId){ 
	        	let url = cont.campus+"v1/campus/coursechain/selectByClassPeriodIdAndFirstLessonDate?accessToken=aaaa"
        		let propmes = qs.stringify({
        			coursePeriodId : id//this.schoolId// //
        		})
        		
        		this.axios.post(url , propmes)
        		.then(res=>{    
        				this.listData.addArray.data.listOne = res.data.messages.CoursePeriod
        				this.listData.addArray.data.listTwo = res.data.messages.coursePeriodSetting
        				this.listData.addArray.data.give_item = res.data.messages.coursePeriodSetting.give_item
        				this.listData.addArray.data.classCourse = res.data.messages.classCourse
        				this.listData.addArray.data.fy = res.data.messages.coursePeriodSetting.standardFee
        				this.listData.addArray.codeId = res.data.messages.code
        				this.listData.addArray.oldcodeId = res.data.messages.oldCode
						...
						let newdata1  = new Date().getYear().toString().substring(1)
							...
							let newDataArr = []
							let newDataArr1 = []
							res.data.messages.CoursePeriod.time_label.map((ele,index)=>{
								let newDataObj = {}
								let newDataObj1 = {}
		...
		mounted(){ 
			...
			/**
				初步请求科目列表
			 */
			let titleInitUrl = cont.campus+"v1/campus/coursechain/findSubjectList?accessToken=aaaa"
			let titleInitparmes = qs.stringify({ 
			...
			.then(res => {   
					...
					this.listData.subjectList = res.data.messages.data 
                },
                res => {
                    alert("网络繁忙 ！请刷新重试 ！")
                })
