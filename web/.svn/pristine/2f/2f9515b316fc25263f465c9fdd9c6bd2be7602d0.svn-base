	...
	<div class="szongbu">
		...
		<div class="bumen">
			...
			<ul class="clearfix xuan">
				<li v-for="(item,index) in getAreaListDate" @click.stop.prevent="getDeptList(index)" :key="index" :id="item.area_id" :class="{isActive:item.showDuigou}">{{item.area_name}}</li>
			</ul>
		</div>
		<!--选择一级部门-->
	...
	export default {
		...
		methods: {
			...
			getDeptList(index) {
				...
				let url = cont.system + "v1/system/dept/getDeptList?accessToken=aaaa&headSchoolId=" + this.headSchoolId + "&campusType=3";
				this.axios.post(url).then((getRes) => {
					if(getRes.data.messages.data) {
						let editTree = getRes.data.messages.data;
						for(let i in editTree) {
