    ...
    <div class="campusTopic">
        ...
        <div class="list_bootom" v-if="showDate == 'listPage'">
            ...
            <div class="contentBigBox">
                <div class="contentBlock" v-for="(ele,index) in schoolTopicList" :key="index" :id="ele.schoolTopicId" @click.stop.prevent="showDetail(ele.schoolTopicId)">
                    <div class="contentLeft">
                        <div class="cTitle">{{ele.title}}</div>
                        <div class="cOperation">
                            <span class="del" @click.stop.prevent="deleteSchoolTopic(ele.schoolTopicId)">删除</span><span class="partake">{{ele.memberCount}}人参与</span><span class="time">{{ele.createTime}}</span>
                        </div>
                    </div>
        ...
        <div class="list_bootom detailPage" v-if="showDate =='detailPage'">
            <div class="detailPageBox">
                ...
                <div class="topContent" :id="getDetialDate.schoolTopicId">
                    ...
                    <div class="tOperation">
                        <span class="del" @click.stop.prevent="deleteSchoolTopic(getDetialDate.schoolTopicId)">删除</span><span class="partake">{{getDetialDate.memberCount}}人参与</span><span class="time">2015-10-12</span>
                    </div>
                    <div class="imgBox">
            ...
            <topic-comment :oneLevelComment="oneList" v-if="oneList"></topic-comment>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                topicContent: '',//话题内容
                imgArr:[],//页面图片回显
                newimgurl: cont.aliyunoss+'ossController/uploadImageOss?accessToken=aaaa&folder=header/'
            }
        },
        mounted() {
        ...
        methods: {
            ...
            //校园话题列表
            findSchoolTopicList() {
            	this.$store.dispatch('findSchoolTopicList')
            },
            //删除功能
            deleteSchoolTopic(id) {
                this.$confirm('您确定要删除吗？','提示',{
                	...
                	this.$store.dispatch('delSchoolTopicList',id)
            ...
            saveSchoolTopic() {
                if (!!this.topicTitle && !!this.topicContent && !!this.imgUrl) {
					...
					this.$store.dispatch('saveSchoolTopic',obj)
					this.cancelSchoolTopic()
                } else {
                    return
                }
            ...
            //查看新闻详情页
            showDetail(id) {
            	this.$store.dispatch('getSchoolTopic',id)
              ...
              /*  let url = cont.interconnection + 'v1/schoolTopic/getSchoolTopic?accessToken=aaaa&schoolTopicId=' + id
                this.axios.post(url).then((showRes) => {
                    if (showRes.data.messages.data) {
                        ...
                        this.$store.dispatch('getSchoolTopicOneLeaveComment',id)
                    } else {
                        this.getDetialDate = []
                    }
        ...
        computed:{
        	...mapState({
        		...
        		schoolTopicList:state => state.schoolTopic.schoolTopicList,
        		getDetialDate:state => state.schoolTopic.getDetialDate
        	})
        }
    }
</script>