	...
	<div class="szongbu">
		...
		<div class="bumen">
			...
			<ul class="clearfix xuan">
				<li v-for="(item,index) in getAreaListDate" @click.stop.prevent="getSchoolList(index,item.area_id)" :key="index" :id="item.area_id" :class="{isActive:item.showDuigou}">{{item.area_name}}</li>
			</ul>
		</div>
		<!--选择校区-->
		<div class="bumen" v-if="getSchoolListDate.length>0">
			<h3>校区</h3>
			<ul class="clearfix xuan">
				<li v-for="(item,index) in getSchoolListDate" @click.stop.prevent="getDeptList(index)" :id="item.school_id" :class="{isActive:item.showDuigou}">{{item.school_name}}</li>
			</ul>
		</div>
		<!--选择一级部门-->
		<div class="bumen" v-if="getdepaOneDate.length>0">
			<h3>部门</h3>
			<ul class="clearfix xuan">
...
<script>
	...
	export default {
		...
		methods: {
			//获取总校下的地区
			getAreaList() {
				this.getAreaListDate=[]//初始化地区的数据
				this.getSchoolListDate=[]//初始化校区的数据
				this.getdepaOneDate= [] //初始化一级部门数据
				this.getdepaTwoDate=[] //初始化二级部门数据
				this.getdepaThreeDate= [] //初始化三级部门数据
				this.getRoleDate=[]//初始化取角色数据
				let url = cont.system + "v1/system/dept/getAreaList?accessToken=aaaa&headSchoolId=" + this.headSchoolId + "&regionId=";
			...
			getSchoolList(index,areaId) {
				...
				this.getRoleDate=[]//初始化取角色数据
				//设置地区背景色，切换
				for(let a = 0; a < this.getAreaListDate.length; a++) {
					this.getAreaListDate[a].showDuigou = false
				}
				this.getAreaListDate[index].showDuigou = true;
				let url = cont.system + "v1/system/dept/getSchoolList?accessToken=aaaa&headSchoolId=" + this.headSchoolId + "&areaId=" + areaId;
				this.axios.post(url).then((getRes) => {
					if(getRes.data.messages.data) {
						let editTree = getRes.data.messages.data;
						for(let i in editTree) {
						...
						this.getSchoolListDate = editTree;
					}
				}).catch(function(error) {
					console.log(error)
				});
			...
			getDeptList(index) {
				...
				this.getRoleDate=[]//初始化取角色数据
				//设置地区背景色，切换
				for(let a = 0; a < this.getSchoolListDate.length; a++) {
					this.getSchoolListDate[a].showDuigou = false
				}
				this.getSchoolListDate[index].showDuigou = true;
				let url = cont.system + "v1/system/dept/getDeptList?accessToken=aaaa&headSchoolId=" + this.headSchoolId + "&campusType=4";
				this.axios.post(url).then((getRes) => {
					if(getRes.data.messages.data) {
						let editTree = getRes.data.messages.data;
						for(let i in editTree) {
