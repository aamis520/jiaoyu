	...
	<div class="account">
		<ul class="ul_list">
			<li v-for="(ele,index) in tableData" :key="index">
				...
				<span> 
          <img v-if="ele.teacer_type === 'fullTime'" src="../../../../static/home-icon/qzbg.png"></img>
          <img v-else-if="ele.teacer_type === 'partTime'" src="../../../../static/home-icon/jzbg.png"></img>
          <!--实习和培训图片没有  -->
          <!-- <img v-else-if="ele.teacer_type === 'practice'" src="../../../../static/home-icon/jz.png"></img>
                  <img v-else-if="ele.teacer_type === 'Training'" src="../../../../static/home-icon/jz.png"></img> -->
          <div v-else></div>
          {{ele.full_name}}
        </span>
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			pageInit(num) {
				let pageInitUrl = cont.campus + "v1/campus/account/findCheckedAccount?accessToken=aaaa&head_id=" + this.headSchoolId
				let pageInitparmes = qs.stringify({
					checked: true,
					pageSize: 10,
					pageNo: num,
					headSchoolId: this.headSchoolId,
					school_id: "5a00658e43a7f91e108b6ced"
				})

				this.axios.post(pageInitUrl, pageInitparmes)
					.then(res => {
							this.tableData = res.data.messages.accountList
							this.total = res.data.messages.totalCount
						},
						res => {
							alert("网络繁忙 ！请刷新重试 ！")
						})
			...
			examineAjax(userId, isBoolen) {
				return new Promise((resok, reqno) => {
					let pageInitUrl = cont.campus + "v1/campus/account/stopAndEnableAccount?accessToken=aaaa&head_id=" + this.headSchoolId
					let pageInitparmes = qs.stringify({
						id: userId,
						enable: isBoolen
					}) 
